...
impl<T> LinkedList<T> {
    ...
    /// Insert `obj` at the head of the list.
    ...
    /// Insert `obj` at the tail of the list.
    ...
    /// Delete the head of the list and return its value.
    ...
    /// Delete the tail of the list and return its value.
    ...
    fn get_ith_node(node: Option<NonNull<Node<T>>>, index: i32) -> Option<&'static T> {
        node.and_then(|next_ptr| match index {
