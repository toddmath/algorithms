...
///
/// A Treap is a self-balancing binary search tree. It matains a priority value
/// for each node, such that for every node, its children will have lower
/// priority than itself. So, by just looking at the priority, it is like a
/// heap, and this is where the name, Treap, comes from, Tree + Heap.
...
impl<T: Ord> Treap<T> {
    ...
    /// Does this [`Treap`] contain the given `value`?
...
/// Recursive helper function for [`Treap::insert`].
...
/// Recursive helper function for [`Treap::remove`]
