...
/// UnionFind data structure
/// It acts by holding an array of pointers to parents, together with the size
/// of each subset
#[derive(Debug)]
pub struct UnionFind<T: Hash> {
    /// we are going to manipulate indices to parent, thus `usize`. We need a
    /// map to associate a value to its index in the parent links array
    payloads: HashMap<T, usize>,
    /// holds the relationship between an item and its parent. The root of a set
    /// is denoted by parent_links[i] == i
...
impl<T: Hash> UnionFind<T> {
    ...
    /// Returns the key of an item stored in the data structure or None if it
    /// doesn't exist
    ...
    /// returns None if either value_1 or value_2 hasn't been inserted in the
    /// data structure first returns Some(true) if two disjoint sets have
    /// been merged returns Some(false) if both elements already were
...
impl<T: Hash> Default for UnionFind<T> {
    fn default() -> Self {
        Self {
...
impl<T: Hash + Eq> FromIterator<T> for UnionFind<T> {
