...
// pub trait HeapSort<T: Ord> {
//     /// # Algorithm
//     ///
//     /// Heap sort has two steps:
//     ///   1. Convert the input array to a max heap.
//     ///   2. Partition the array into heap part and sorted part. Initially
// the     ///      heap consists of the whole array and the sorted part is
// empty:     ///      ```text
//     ///      arr: [ heap                    |]
//     ///      ```
//     ///
//     ///      Repeatedly swap the root (i.e. the largest) element of the heap
//     /// with      the last element of the heap and increase the sorted part
//     /// by one:      ```text
//     ///      arr: [ root ...   last | sorted ]
//     ///       --> [ last ... | root   sorted ]
//     ///      ```
...
//     fn heapify(&mut self);
//     fn move_down(&mut self, root: usize);
// }

// impl<T: Ord> HeapSort<T> for [T] {
//     fn move_down(&mut self, mut root: usize) {
//         let last = self.len() - 1;
//         loop {
//             let left = 2 * root + 1;
//             if left > last {
...
//             }
//             let right = left + 1;
//             let max = if right <= last && self[right] > self[left] {
//                 right
//             } else {
//                 left
//             };

//             if self[max] > self[root] {
//                 self.swap(root, max);
//             }
//             root = max;
//         }
