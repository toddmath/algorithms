...
pub use self::{
    heap::heap_permute,
    naive::{permute, permute_unique},
    steinhaus_johnson_trotter::steinhaus_johnson_trotter_permute,
};

pub type Permuted<T> = Vec<Vec<T>>;
...
mod tests {
    ...
    pub(crate) fn assert_valid_permutation(original: &[i32], permuted: &[i32]) {
        ...
        for permute_value in permuted {
            let count = indices.get_mut(permute_value).unwrap_or_else(|| {
                panic!(
            ...
            *count -= 1; // use this value
            if *count == 0 {
                indices.remove(permute_value); // so that we can simply check every value has been removed properly
            }
        }
    ...
    /// Holds a Vec<i32> with just a few items, so that it's not too long to
